...
2. **Install dependencies**.  The included `Dockerfile` copies the
   `dataflow_common` source code into the image and installs the
   dependencies listed in `requirements.txt` using `pip`.  It then
   exposes the package via the `PYTHONPATH` environment variable.  You
   do **not** need to run `pip install` on the library itself; Python
   will find it in the container because `PYTHONPATH` points to the
   copied directory.  Feel free to modify `requirements.txt` to include
   the two dependencies you mentioned (for example `google-cloud-bigquery`
   and `pyarrow`).
